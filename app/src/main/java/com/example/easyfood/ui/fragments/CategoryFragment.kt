...
// Kategori listesini göstermek için kullanılan fragment sınıfı
class CategoryFragment : Fragment(R.layout.fragment_category) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Kategori adaptörü ve ViewModel'i oluştur
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        observeCategories()
        // Kategoriye tıklama olayı ayarlanıyor
        onCategoryClick()
    }

    // Kategoriye tıklandığında yapılacak işlemi tanımlayan metot
    private fun onCategoryClick() {
        myAdapter.onItemClicked(object : CategoriesRecyclerAdapter.OnItemCategoryClicked {
            override fun onClickListener(category: Category) {
                // Yeni aktiviteye geçiş yapılıyor ve seçilen kategori gönderiliyor
    ...
    // Kategorileri gözlemleyen ve RecyclerView'e bağlayan metot
    private fun observeCategories() {
        categoryMvvm.observeCategories().observe(viewLifecycleOwner, object : Observer<List<Category>> {
            override fun onChanged(t: List<Category>?) {
                // Kategori listesini güncelleyen metot
